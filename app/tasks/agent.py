...
def execute_agent_action_task(action_id: str):
    ...
    try:
        ...
        # Re-fetch to honour a user-initiated cancel that arrived during execution
        db.session.expire(action)
        action = AgentAction.query.get(action_id)
        if action.status == AgentAction.STATUS_FAILED and action.error_message == 'Paused by user':
            logger.info('AgentAction %s was paused during execution — discarding result', action_id)
            return
